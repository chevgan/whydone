...
- If `ROOT/.whydone/` does not exist: resolve the CLI by probing the chain above with the harmless `<CLI> --version` (first chain position that prints a version wins — never probe with a writing command). If a CLI resolved, ask the user ONE question: `No journal in this repo yet — scaffold .whydone/ (journal dir + config) and add a whydone marker block to CLAUDE.md, then continue logging? (committed — ordinary repo files, shared with the team / local — hidden from git via .git/info/exclude, this machine only / cancel)`. On `committed`, run `<CLI> init --journal-only` from ROOT; on `local`, run `<CLI> init --journal-only --local` from ROOT. If init exits non-zero, show its output verbatim and STOP — never fall through to writing the entry into a half-made scaffold. On success, continue with the fresh scaffold. On cancel, stop. If no CLI resolved, STOP and tell the user to run `npx whydone init` first. Never create `.whydone/` or its files yourself — init owns bootstrap.
