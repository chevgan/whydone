...
**Auto mode is silent.** Nothing is printed before the write — no preview, no question; you get a one-line report naming the file. The entry lands as an ordinary uncommitted file: review it in `git diff` (or open the file for local-storage journals), delete it to reject. Ambiguous cases (same-day overlap, superseded entries, no git repo, size-detected minimal entries) still fall back to the full preview-and-ask flow even in auto.
